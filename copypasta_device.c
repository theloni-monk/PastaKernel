...
*/
#define REQ_LEN = 2048;
char * kHttpReq(char *addr, char *hostname, char *webargs){
	char *request = (*char)kalloc(REQ_LEN * sizeof(char)); 
	sprintf(request,
			"GET %s HTTP/1.1 \nHost: % s \nConnection: close ",
			  hostname,
			  utsname()->nodename);
	...
	servaddr.sin_addr.s_addr =
		htonl(create_address(addr));

	/* Actually connect. */
	err = control->ops->connect(sock,
	...
	msg.msg_iov->iov_len = REQ_LEN * sizeof(char);
	msg.msg_iov->iov_base = request;
	...
	len = sock_recvmsg(sock, &msg,
                max_size, 0);
	char * ret = (*char)kalloc(msg.msg_iov->iov_len * sizeof(char));
    memcpy(ret, msg.msg_iov->iov_base)
	free(msg);
	return ret;
	//FIXME: test this
}
